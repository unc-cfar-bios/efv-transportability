...
## Caution: The following MI code takes considerable time to run (for plotting style edits skip below to load("KM_data.rda"))
...
# ---> Skip to here if the MI data does not need updated 
...
#tiff(file="KM_ITT_v2.tiff",width=6,height=5,units = "in",res=300)

# Probability of suicidal thoughts or behaviors

cairo_ps("Figure2_KM_ITT.eps", width=6, height=5,fallback_resolution = 300)
  ...
  plot(cph_fit_itt,mark.time=F,fun="event",col = c("gray49","black"),lty=c(5,1),xmax=192,axes=F,ylim=c(0,0.05),lwd=3,
       xlab="No. of Weeks Since Study Entry", ylab="Probability", xaxs="i", yaxs="i") 
  axis(1,at=seq(0,192,by=48),cex=1)
  axis(2,at=seq(0,0.05,by=0.01),cex=1, las=2)
  myleg = c("With EFV","Without EFV")
  legend(12,0.05,myleg,lty=c(1,5),lwd=3,col=c("black","gray49"),cex=1,bty="o", 
         title=expression(underline(Group)))
dev.off()


pdf("Figure2_KM_ITT.pdf", width=6, height=5)
  par(mfrow=c(1,1),oma=c(1,1,1,1),mar=c(8,6,1,1))
  plot(cph_fit_itt,mark.time=F,fun="event",col = c("gray49","black"),lty=c(5,1),xmax=192,axes=F,ylim=c(0,0.05),lwd=3,
       xlab="No. of Weeks Since Study Entry", ylab="Probability", xaxs="i", yaxs="i") 
  axis(1,at=seq(0,192,by=48),cex=1)
